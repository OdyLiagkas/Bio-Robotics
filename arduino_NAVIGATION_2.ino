...
void setup() {
  ...
  tone(BUZZER_PIN, 1000, 200);   // Beep 1kHz for 200ms to indicate startup success

  // --- IMU and Motion Tracking Setup ---
  trackRotationStart();
  ...
  // ==========================================================
  // === YOUR ONE-TIME AUTOMATIC SEQUENCE AFTER INITIALIZATION ===
  // ==========================================================

  int block_array[] = {6, 12, 2, 10, 4, 14}; // FINAL BLOCK ALWAYS 14!!

  // Optional: run a short delay before the motion begins
  delay(1000);

  Serial.println("Starting automatic block sequence...");

  trackRotationUpdate();  // Make sure IMU is fresh before moving
  sequence_of_moves(block_array);  // Run your full motion sequence

  // Reset position variables
  x = 0; y = 0; fx = 0; fy = 0; dx = 0; dy = 0;

  Serial.println("Sequence completed once.");

  // Optional: flash LEDs green to indicate completion
...
void loop() {
  // Do nothing — sequence runs once after boot
}
